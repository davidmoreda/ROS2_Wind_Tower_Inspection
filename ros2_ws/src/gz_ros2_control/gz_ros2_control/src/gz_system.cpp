  ...
  unsigned int update_rate)
{
  ...
  for (unsigned int j = 0; j < this->dataPtr->joints_.size(); j++) {
    ...
    auto joint_type_comp = _ecm.Component<sim::components::JointType>(simjoint);
    if (!joint_type_comp) {
      RCLCPP_WARN_STREAM(
        this->nh_->get_logger(),
        "Joint '" << joint_name << "' has no JointType component in ECM, skipping.");
      continue;
    }
    this->dataPtr->joints_[j].joint_type = joint_type_comp->Data();

    auto joint_axis_comp = _ecm.Component<sim::components::JointAxis>(simjoint);
    if (!joint_axis_comp) {
      RCLCPP_WARN_STREAM(
        this->nh_->get_logger(),
        "Joint '" << joint_name << "' has no JointAxis component in ECM, skipping.");
      continue;
    }
    this->dataPtr->joints_[j].joint_axis = joint_axis_comp->Data();
